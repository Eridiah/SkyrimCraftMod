...
public class Smeltery extends BlockContainer {
	...
	public Smeltery(boolean blockState) {
		super(Material.rock);
		rand = new Random();
		isActive = blockState;
    ...
    {
        this.blockIcon = iconRegister.registerIcon(Reference.modID + ":" + "SmelterSide");
        this.iconTop = iconRegister.registerIcon(Reference.modID + ":" + (this.isActive ? "SmelterTopOn" : "SmelterTopOff"));
    }
    
    //^^ I used a new top Icon to configure 3 different textures rather than two!
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entityPlayer, ItemStack itemstack) {
		...
		if(itemstack.hasDisplayName()) {
			//((TileEntityBlastFurnace)world.getTileEntity(x, y, z)).setCustomName(itemstack.getDisplayName());
		}
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
		if(world.isRemote) {
			...
			TileEntitySmeltery entity = (TileEntitySmeltery) world.getTileEntity(x, y, z);
			if(entity != null) {
				FMLNetworkHandler.openGui(player, SkyrimCraft.instance, SCDecs.guiIDSmeltery, world, x, y, z);
			}
			return true;
		}else{
			return false;
	...
	public static void updateBlockState(boolean isCooking, World world, int xCoord, int yCoord, int zCoord) {
		...
		if(isCooking) {
			world.setBlock(xCoord, yCoord, zCoord, SCDecs.blockSmelteryActive);
		}else{
			world.setBlock(xCoord, yCoord, zCoord, SCDecs.blockSmelteryIdle);
		}
